<article class="w-full bg-white flex flex-col shadow my-4">
    ...
    <div class="bg-white flex flex-col justify-start p-6">
        <div class="flex gap-4">
            @foreach ($post->categories as $categoryRecent)
                <a href="{{ route('categories.show', $categoryRecent) }}"
                    class="text-blue-700 text-sm font-bold uppercase pb-4">{{ $categoryRecent->title }}</a>
            @endforeach
        </div>

        <div class="flex justify-between items-center pb-4">
            <a href="{{ route('posts.show', $post) }}"
                class="text-3xl font-bold hover:text-gray-700 ">{{ $post->title }}</a>
        ...
        <a href="{{ route('posts.show', $post) }}" class="uppercase text-gray-800 hover:text-black">Continue Reading <i
